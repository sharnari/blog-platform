...
export const getCurrentUser = createAsyncThunk('users/current', async (token) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(registerUser.rejected, (state, action) => {
        state.statusLoading = null
        state.error = action.error.message.includes('422') ? 'Такой пользователь уже существует!' : action.error.message
      })
